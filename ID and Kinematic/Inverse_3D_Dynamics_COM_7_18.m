...
function [Knee, Ankle, markerStruct, forceStruct] = Inverse_3D_Dynamics_COM_7_18(markerStruct, forceStruct, static_markerStruct, anthropometrics, trial_name)
    ...
    disp('static_markerStruct');
    disp(static_markerStruct);
    
    temp = static_markerStruct.LLatMal;
    disp = (temp);
    
    if ~isfield(static_markerStruct, 'LLatMal')
            disp('ERROR in Inverse_3D_Dynamics: LLatMal field is missing from static_markerStruct!');
            disp('Available fields:');
            %disp(fieldnames(static_markerStruct));
    end
    
    [right_globe2ana,left_globe2ana,right_seg_omega,left_seg_omega,right_seg_alpha,left_seg_alpha,right_ang,right_omega,right_alpha,left_alpha] = calibrationmatrix_6_27_debug(static_markerStruct,markerStruct);
    ...
    for i = 1:length(fields)
        forceStruct.(fields{i})(:,1:3) = filtfilt(bb, ba, forceStruct.(fields{i})(:,1:3));
    end
    ...
    Ankle.Left.Force = zeros(3,length(forceStruct.f1))';
    Ankle.Right.Force = zeros(3,length(forceStruct.f1))';
    Ankle.Left.Moment = zeros(3,length(forceStruct.f1))';
    Ankle.Right.Moment = zeros(3,length(forceStruct.f1))';
    
    % Calculate x-Force (Med/Lat)
    Ankle.Left.Force(:,1) = Foot.Mass*Acceleration.Left.Foot(:,1) - forceStruct.f1(:,1);
    Ankle.Right.Force(:,1) = Foot.Mass*Acceleration.Right.Foot(:,1) - forceStruct.f2(:,1);
    
    % Calculate y- and z- Force for different slopes (Fore/Aft & Vertical)
    if contains(lower(trial_name), 'uphill')
        Ankle.Left.Force(:,2) = Foot.Mass*Acceleration.Left.Foot(:,2) - forceStruct.f1(:,2) + Foot.Mass*9.81*cos(8.6*pi/180);
        Ankle.Right.Force(:,2) = Foot.Mass*Acceleration.Right.Foot(:,2) - forceStruct.f2(:,2) + Foot.Mass*9.81*cos(8.6*pi/180);
        Ankle.Left.Force(:,3) = Foot.Mass*Acceleration.Left.Foot(:,3) - forceStruct.f1(:,3) + Foot.Mass*9.81*sin(8.6*pi/180);
        Ankle.Right.Force(:,3) = Foot.Mass*Acceleration.Right.Foot(:,3) - forceStruct.f2(:,3) + Foot.Mass*9.81*sin(8.6*pi/180);
    ...
    elseif contains(lower(trial_name),'neutral')
        Ankle.Left.Force(:,2) = Foot.Mass*Acceleration.Left.Foot(:,2) - forceStruct.f1(:,2) + Foot.Mass*9.81;
        Ankle.Right.Force(:,2) = Foot.Mass*Acceleration.Right.Foot(:,2) - forceStruct.f2(:,2) + Foot.Mass*9.81;
        Ankle.Left.Force(:,3) = Foot.Mass*Acceleration.Left.Foot(:,3) - forceStruct.f1(:,3);
        Ankle.Right.Force(:,3) = Foot.Mass*Acceleration.Right.Foot(:,3) - forceStruct.f2(:,3);
    end
    ...
    Ankle.Left.Moment(:,2) = Foot.MOI(2)*foot_left_alpha(:,2)+ ...
        ...
        (Ankle.Left.Force(:,1) .* (l_ajc(:,3)-LFoot.COM(:,3)))-forceStruct.m1(:,2);
    % Abduction
    ...
    Ankle.Right.Moment(:,2) = Foot.MOI(2)*foot_right_alpha(:,2)+ ...
        ...
        (Ankle.Right.Force(:,1) .* (r_ajc(:,3)-RFoot.COM(:,3)))-forceStruct.m2(:,2);
    %Abduction
    ...
    Knee.Left.Force = zeros(length(forceStruct.f1),3);
    Knee.Right.Force = zeros(length(forceStruct.f1),3);
    Knee.Right.Moment = zeros(length(forceStruct.f1),3);
    Knee.Right.Moment = zeros(length(forceStruct.f1),3);
