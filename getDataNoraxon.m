    ...
    found = false;
        ...
        for j = 1:length(trialnames_TRUE)
            ...
            for i = 1:length(trialnames)
                if (contains(trialnames(i),pat,'IgnoreCase',false)) %also accounts for noncase sensitive patterns (you need both)
                    %disp(trialnames(i) + " file matched to trialnames_TRUE: " + trialnames_TRUE(j)); %troubleshooting
                    path = strcat(path_Noraxon,trialnames(i));
                    data_Small = DataNoraxonConcat (path);
                    data_Noraxon {j} = data_Small;
